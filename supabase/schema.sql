...
-- The original unit's destination depends on the return reason (Defective
-- Unit / Not as Described -> Supplier Defective, everything else -> back
-- to Available for resale) — a changed-mind return isn't a broken unit.
-- p_new_price lets staff set the swap's actual sale price instead of
-- silently keeping the old unit's price when the replacement is a
-- different (often differently priced) model.
...
-- replace_return) without requiring a replacement to be picked yet —
-- for when a defective unit needs to go back to the supplier before
-- restock arrives to actually fulfill the customer's replacement.
...
-- only handles the replacement side.
...
-- supplier_defective_records/cgn_resales all reference devices without
-- cascading. This cleans up that history first (including any CGN Ledger
-- entry pointing at it — deleted along with the device rather than left
-- dangling with a null device_id), then removes the device — same
...
insert into public.product_models (category, name, colors) values
  ...
  ('iPhone', 'iPhone 11 Promax', ARRAY['Midnight Green','Space Gray','Silver','Gold']),
  ...
  ('iPhone', 'iPhone 12 Promax', ARRAY['Graphite','Silver','Gold','Pacific Blue']),
  ...
  ('iPhone', 'iPhone 13 Promax', ARRAY['Graphite','Gold','Silver','Sierra Blue','Alpine Green']),
  ...
  ('iPhone', 'iPhone 14 Promax', ARRAY['Space Black','Silver','Gold','Deep Purple']),
  ...
  ('iPhone', 'iPhone 15 Promax', ARRAY['Black Titanium','White Titanium','Blue Titanium','Natural Titanium']),
  ...
  ('iPhone', 'iPhone 16 Promax', ARRAY['Black Titanium','White Titanium','Natural Titanium','Desert Titanium']),
  ...
  ('iPhone', 'iPhone 17 Promax', ARRAY['Silver','Cosmic Orange','Deep Blue']),
